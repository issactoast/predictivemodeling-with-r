...
housing_recipe <- all_data %>% 
  recipe(sale_price ~ .) %>%
  step_rm(id, misc_feature, pool_qc, fireplace_qu) %>% 
  step_log(sale_price) %>% 
...
rf_m <- randomForest(sale_price ~ ., data = train2, importance = TRUE, proximity = TRUE)
