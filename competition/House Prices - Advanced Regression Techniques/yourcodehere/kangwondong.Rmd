...
```{r, message=FALSE}
train <- read.csv(file.path(file_path, "train.csv"))
test <- read.csv(file.path(file_path, "test.csv"))
```
...
```
##modimpute가 na를 채우는 방법-->가장 빈번한 예로 NA를 채워라

## `juice` the all_data2 and split(전처리한거 빼내기)
...
#tidyverse{step_dummy==data.model}

```{r}

f<-function(i){
 ggplot(data=train,aes(x=train[[i]],y=train[[81]]))+geom_boxplot()
}
g<-function(i){ggplot(data=train,aes(x=train[[i]],y=train[[81]]))+geom_point()
}
...
g(37)
```
##this variable does not have a lot impact on the Saleprice
```{r}
g(46)
```
##this variable does not have a lot impact on the Saleprice
...
lm_form_fit <- 
    lm_model %>% 
    fit(log(sale_price) ~.-bsmt_fin_sf2-low_qual_fin_sf-bsmt_half_bath-pool_area-misc_val-yr_sold-x3ssn_porch, data = train2)
...
```{r warning=FALSE}
result <- exp(predict(lm_form_fit, test2))
result %>% head()
```
