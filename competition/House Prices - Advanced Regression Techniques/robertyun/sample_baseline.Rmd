...
**ols_step_backward_aic를 적용하고 싶었지만 dummy가 포함된 데이터 타입에는 적용하는 법을 몰라서
  imputation만 적용한 데이터를 얻겠다.**
...
# Set linear regression model and exercise ols_step_backward_aic to select a model

```{r, warning = FALSE}
fit = lm(sale_price ~ ., train2)
k = ols_step_forward_aic(fit)
...
submission$SalePrice <- exp(predict(k$model,test2))
write.csv(submission, row.names = FALSE,
          "Second_Submission_윤상진.csv")
```
